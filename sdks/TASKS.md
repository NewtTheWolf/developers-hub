...
| 2.5 | Publish `@turbosmtp/sdk` to npm | BLOCKED | | Depends on 2.3–2.4. **`package.json` decisions to settle first (discuss before editing):** (1) `repository.url` currently an invented `turbosmtp-node.git` → set real repo + `directory` subpath if it stays in `developers-hub`; (2) **add `publishConfig.access:"public"`** — scoped `@turbosmtp/*` defaults to private, this is a hard publish blocker; (3) module format — CJS-only (current) vs dual ESM+CJS (`exports` map); (4) `files` negation `!src/**/*.test.ts` is unreliable → tidy with test-location choice (tests currently in `test/`, not shipped); (5) confirm `license:MIT` is intended (business call). See conversation 2026-07-28/31. |
